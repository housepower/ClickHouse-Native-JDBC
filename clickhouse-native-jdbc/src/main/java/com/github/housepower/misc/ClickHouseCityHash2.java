 ...
 */
public class ClickHouseCityHash2 {
    ...
    private static long fetch32(ByteBuf s, int pos) {
        return s.getIntLE(pos);
    }

    private static long fetch64(ByteBuf s, int pos) {
        return s.getLongLE(pos);
    }
    ...
    private static long hashLen0to16(ByteBuf s, int pos, int len) {
        if (len > 8) {
        ...
        if (len > 0) {
            byte a = s.getByte(pos);
            byte b = s.getByte(pos + (len >>> 1));
            byte c = s.getByte(pos + len - 1);
            int y = (int) a + (((int) b) << 8);
            int z = len + (((int) c) << 2);
    ...
    private static long[] weakHashLen32WithSeeds(ByteBuf s, int pos, long a, long b) {
    ...
    private static long[] cityMurmur(ByteBuf s, int pos, int len, long seed0, long seed1) {
    ...
    private static long[] cityHash128WithSeed(ByteBuf s, int pos, int len, long seed0, long seed1) {
        if (len < 128) {
    ...
    public static long[] cityHash128(ByteBuf s, int pos, int len) {
        if (len >= 16) {
