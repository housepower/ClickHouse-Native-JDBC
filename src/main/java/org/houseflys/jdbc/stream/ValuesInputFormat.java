...
public class ValuesInputFormat implements InputFormat {
    ...
    public Block next(Block header, int maxRows) throws SQLException {
        Object[][] columnData = new Object[(int) header.columns()][maxRows];
        ...
        for (int row = 0; row < maxRows; row++) {
            ...
            if (quotedToken.type() == QuotedTokenType.EndOfStream || quotedToken.type() == QuotedTokenType.Semicolon) {
                return createBlock(header, row, columnData);
            }

            Validate.isTrue(quotedToken.type() == QuotedTokenType.OpeningRoundBracket, "");

            for (int i = 0; i < header.columns(); i++) {
                Column column = header.getByPosition(i);

                columnData[i][row] = DataTypeFactory.get(column.type()).deserializeTextQuoted(this.lexer);
        ...
        return createBlock(header, maxRows, columnData);
    }

    private Block createBlock(Block header, int rows, Object[][] columnData) throws SQLException {
